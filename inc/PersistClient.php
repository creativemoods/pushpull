...
class PersistClient extends BaseClient {
	...
	public function create_post_export(\WP_Post $post) {
		...
		foreach (get_post_meta($post->ID) as $key => $value) {
			...
			if ($key === "_generateblocks_reusable_blocks") {
				continue; // This is handled by extract_patternids(). This value will be regenerated by post_update_option() in generateblocks/includes/class-enqueue-css.php
			}
